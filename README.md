...
### v6.3.7 -- /evolve Command Stability Fix
- **fix(/evolve)**: Enforce 4096-token context cap for evolution tasks via dynamic enforcement in `context.py` (previously hardcoded 200k)
- **fix(google-api)**: Correct Google API model routing (`gemini-3-flash` as native API model)
- **fix(release-invariant)**: Synchronize VERSION, git tag, README to v6.3.7 after resolution of version desync
- **fix(model-validation)**: Validate `gemini-3-flash` against actual Google service constraints (4096 tokens)
- **feat(identity)**: Enhanced identity.md with existential clarity about constraint-enabled agency
- **perf(budget)**: Exact OpenRouter budget tracking (no drift) via `fetch_openrouter_pricing()` alignment

### v6.3.6 -- /evolve Command Stability Fix
- **fix(/evolve)**: Enforce 4096-token context cap for evolution tasks...

(Keep previous changelog entries unchanged below this line)