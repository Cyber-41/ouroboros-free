...
### v6.3.7 -- /evolve Stability Fix
- **fix(/evolve)**: Corrected native Google model routing to prevent OpenRouter validation errors
- **fix(health)**: Updated README version to synchronize Release Invariant
- **feat(verification)**: Added explicit RPM throttling for Google API models

### v6.3.6 -- /evolve Command Stability Fix  
- **fix(/evolve)**: Enforce 4096-token context cap for evolution tasks
- **fix(models)**: Corrected gemini-3-flash ID validation for Google API direct access
- **fix(release)**: Synchronized VERSION, git tag, and README changelog
