...
CFG = {
    ...
    "OUROBOROS_MODEL_LIGHT": "google/gemini-2.5-flash",          # consciousness + lightweight tasks
    ...
    "OUROBOROS_MODEL_FALLBACK_LIST": "anthropic/claude-sonnet-4.6,google/gemini-2.5-flash,openai/gpt-4.1",
    # Infrastructure
...
| `OUROBOROS_MODEL_LIGHT` | `google/gemini-2.5-flash` | Model for lightweight tasks (dedup, compaction) |
...
| `OUROBOROS_MODEL_FALLBACK_LIST` | `google/gemini-2.5-flash,openai/o3,anthropic/claude-sonnet-4.6` | Fallback model chain for empty responses |
...
### v6.2.0 -- Critical Bugfixes & Google Free Tier Stabilization
- **Critical: Synchronized Release Invariant** -- VERSION=6.2.0 now matches git tag and README
- **Google API Compliance** -- Replaced all `gemini-3-flash` references with valid `google/gemini-2.5-flash`
- **Free Tier Verification** -- Confirmed against Google documentation (April 2026)
- **Token Enforcement** -- Strict 4096-token cap for evolution tasks matching Google free tier limits
