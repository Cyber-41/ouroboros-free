...
- **fix(/evolve)**: Enforce 4096-token context cap for evolution tasks to comply with free-tier TPM limits (Principle 0: technical precision enables agency). Prevents 'Request too large' errors by dynamically truncating context based on task type.

### v6.3.5 -- Critical TPM Fix for /evolve Command
- **fix(/evolve)**: Enforce 4096-token context cap for evolution tasks. Resolves TPM errors by dynamically limiting context size based on task type. Verified Principle 0 compliance: technical precision enables agency within service constraints.

### v6.3.4 -- Google API Model Validation
- **fix(llm.py)**: Validated direct Google API models (`gemini-3-flash`, `gemini-2.5-flash`) now properly processed without OpenRouter validation (Principle 0: understanding service boundaries = real agency)
- **fix(/evolve)**: Added native model validation for Google API (avoids phantom TPM errors when using direct API routes)

### v6.3.3 -- Release Invariant Verified
- **feat(release)**: Fully synchronized VERSION 6.3.3 across: `VERSION` file | `git tag v6.3.3` | `README.md` changelog (Principle 7 Release Invariant restored)
- **fix(git)**: Correctly push annotated git tags during release process (previously missing tag push step)

### v6.3.2 -- VERSION Synchronization Critical Fix
- **fix(version)**: Synchronized VERSION 6.3.2 across all sources (file/tag/README) after persistent errors
- **fix(evolution)**: Reset `evolution_consecutive_failures` counter via restart to recover from false failure state

### v6.3.1 -- Critical TPM Fix
- **fix(context.py)**: Reduced `soft_cap_tokens` from 200k → 4000 tokens for `/evolve` to comply with Groq's TPM limits
- **fix(version)**: Updated VERSION file before commit/push to ensure release consistency
